...
const LottoCard = ({ card, drawnNumbers, onMark, marked }: LottoCardProps) => {
  return (
        ...
        {card.map((row, rIdx) => (
          row.map((num, cIdx) => {
            ...
            return (
              <div 
                key={`${rIdx}-${cIdx}`}
                onClick={() => num && isDrawn && onMark(num)}
                className={`
                  ...
                  ${!num ? 'bg-transparent' : 
                    ...
                    isDrawn ? 'bg-amber-400 text-amber-900 cursor-pointer ring-4 ring-amber-600 ring-offset-2 animate-bounce' : 
...
const BigKeg = ({ number }: { number: number | null }) => (
  <AnimatePresence mode="wait">
    {number && (
      <motion.div
        key={number}
        initial={{ y: -300, rotate: -360, scale: 0, opacity: 0 }}
        animate={{ y: 0, rotate: 0, scale: 1, opacity: 1 }}
        exit={{ y: 300, rotate: 360, scale: 0, opacity: 0 }}
        transition={{ type: "spring", damping: 12, stiffness: 100 }}
        className="relative group"
      >
        {/* Shadow */}
        <div className="absolute -bottom-4 left-1/2 -translate-x-1/2 w-32 h-8 bg-black/20 blur-xl rounded-full" />
        
        {/* Keg Body */}
        <div className="w-48 h-48 bg-gradient-to-b from-red-500 to-red-800 rounded-full flex items-center justify-center shadow-2xl border-8 border-red-900 relative">
          {/* Wood Texture Effect */}
          <div className="absolute inset-0 opacity-20 bg-[url('https://www.transparenttextures.com/patterns/wood-pattern.png')] rounded-full" />
          
          {/* Inner Circle */}
          <div className="bg-stone-100 w-32 h-32 rounded-full flex items-center justify-center shadow-[inset_0_4px_10px_rgba(0,0,0,0.4)] border-4 border-stone-300">
            <span className="text-7xl font-black text-red-700 font-mono drop-shadow-md">{number}</span>
          </div>
          
          {/* Highlights */}
          <div className="absolute top-4 left-8 w-12 h-6 bg-white/20 rounded-full blur-sm -rotate-45" />
        </div>
      </motion.div>
    )}
  </AnimatePresence>
